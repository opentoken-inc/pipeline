...
JsonErrno jsonParse(char *s, char **endptr, JsonValue *value, JsonAllocator &allocator) {
    ...
    *endptr = s;
    ...
    while (*s) {
        ...
        *endptr = s++;
        ...
        case '-':
            if (!isdigit(*s) && *s != '.') {
                *endptr = s;
                return JsonErrno::JSON_BAD_NUMBER;
            }
        case '0':
        case '1':
        case '2':
                    ...
                    case 'u':
                        ...
                        for (int i = 0; i < 4; ++i) {
                            if (isxdigit(*++s)) {
                                ...
                                *endptr = s;
                                return JsonErrno::JSON_BAD_STRING;
                            }
                        }
                        if (c < 0x80) {
                            *it = c;
        ...
        case 't':
            if (!(s[0] == 'r' && s[1] == 'u' && s[2] == 'e' && isdelim(s[3])))
                return JsonErrno::JSON_BAD_IDENTIFIER;
        ...
        case ']':
            if (pos == -1)
                return JsonErrno::JSON_STACK_UNDERFLOW;
            if (tags[pos] != JsonTag::JSON_ARRAY)
                return JsonErrno::JSON_MISMATCH_BRACKET;
        ...
        case '}':
            ...
            if (tags[pos] != JsonTag::JSON_OBJECT)
                return JsonErrno::JSON_MISMATCH_BRACKET;
            if (keys[pos] != nullptr)
                return JsonErrno::JSON_UNEXPECTED_CHARACTER;
            o = listToValue(JsonTag::JSON_OBJECT, tails[pos--]);
            break;
        case '[':
            if (++pos == JSON_STACK_SIZE)
                return JsonErrno::JSON_STACK_OVERFLOW;
            tails[pos] = nullptr;
        ...
        if (pos == -1) {
            *endptr = s;
            *value = o;
            return JsonErrno::JSON_OK;
        }

        if (tags[pos] == JsonTag::JSON_OBJECT) {
            if (!keys[pos]) {
                if (o.getTag() != JsonTag::JSON_STRING)
                    return JsonErrno::JSON_UNQUOTED_KEY;
                keys[pos] = o.toString();
                continue;
            }
            if ((node = (JsonNode *) allocator.allocate(sizeof(JsonNode))) == nullptr)
                return JsonErrno::JSON_ALLOCATION_FAILURE;
    ...
    return JsonErrno::JSON_BREAKING_BAD;
}
}  //  namespace
